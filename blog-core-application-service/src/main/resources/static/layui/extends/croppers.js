...
layui.config({
    base: '/layui/extends/' //layui自定义layui组件目录
}).define(['jquery','layer','cropper'],function (exports) {
    var frame;
    var $ = layui.jquery
        ,layer = layui.layer;
    var html = "<link rel=\"stylesheet\" href=\"/layui/extends/cropper.css\">\n" +
        ...
        "        </div>\n" +
        "        <div class=\"layui-form-mid layui-word-aux\">头像的尺寸限定150x150px,大小在50kb以内(非头像不受限制)</div>\n" +
        "    </div>\n" +
        ...
        "                    <button type=\"button\" class=\"layui-btn\" title=\"移动\">X</button>\n" +
        "                    <button type=\"button\" class=\"layui-btn\" title=\"放大图片\">X</button>\n" +
        "                    <button type=\"button\" class=\"layui-btn\" title=\"缩小图片\">X</button>\n" +
            ...
            $(".layui-btn").on('click',function () {
                ...
                if(event === 'confirmSave'){
                    image.cropper("getCroppedCanvas",{
                        ...
                        formData.append('userfile',blob,'head.jpg');
                        $.ajax({
                            type:"post",
                            ...
                            success:function(result){
                                ...
                                if(result.ret){
                                    layer.close(frame);
                                    return done(result.info.md5);
                                }else{
                                    layer.alert(result.error.message,{icon: 2});
                                }
